...
public class Attempts2 : MonoBehaviour
{
    ...
    IEnumerator CreateAttemptsRoutine(string jsonArrayString)
    {
        ...
        for (int i = 1; i < jsonArray.Count; i = i + 2)
        {
            ...
            //Debug.Log("attemptInfoJson[\"level_id\"] = "+attemptInfoJson["level_id"]);

            if (attemptInfoJson["level_id"] == "6")
            {
                attempt.transform.Find("Level").GetComponent<TMP_Text>().text = "endless mode";
                attempt.transform.Find("attempts").GetComponent<TMP_Text>().text = "attempts: " + attemptInfoJson["attempts"];
                attempt.transform.Find("avg_score").GetComponent<TMP_Text>().text = "avg score = " + attemptInfoJson["average_score"] + "m";
                attempt.transform.Find("avg_score").Translate(new Vector3(2.6f, 0, 0), Space.World);
                attempt.transform.Find("max_score").GetComponent<TMP_Text>().text = "max score = " + attemptInfoJson["max_score"] + "m";
                attempt.transform.Find("perfect_completed").GetComponent<TMP_Text>().text = " ";
            }
